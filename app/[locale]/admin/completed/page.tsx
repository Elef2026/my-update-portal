...
import { CheckCircle2, Eye, Printer, FileCheck, DollarSign, Wallet, ArrowDownRight, ArrowUpRight } from "lucide-react";
...
export default function AdminCompletedPage() {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      // Fetch completed orders awaiting Sunday settlement
  ...
  const updateOnlyOrders = orders.filter((o) => o.orderType === "UPDATE_ONLY");
  const printOrders = orders.filter((o) => o.orderType === "FULL_SERVICE");
  ...
  for (const o of orders) {
    ...
    const totalP = Number(o.totalPaid || 0);

    totalShopEarned += shopE;
    totalAdminCut += adminE + fees;

    if (o.paymentMethod === "CASH_TO_SHOP") {
      // Shop collected cash directly from customer
      totalCashCollected += adminE + fees;
    } else {
      // Admin holds online Chapa payment
      totalChapaEarnings += shopE;
    }
  }

  // Net Balance: Positive = Admin pays Shop, Negative = Shop pays Admin
    ...
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card p-6 rounded-xl border shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
            ...
            ያለቁ ስራዎች እና የሳምንቱ ሂሳብ (Completed Tasks & Unsettled Financials)
          </h1>
          <p className="text-muted-foreground mt-1 text-sm">
            ህትመታቸው ወይም አብዴታቸው ተጠናቆ እሁድ ክፍያ ማካካሻ (Settlement) የሚጠበቁ ስራዎች በሙሉ
          </p>
        </div>
        <Button variant="outline" onClick={fetchOrders}>አድስ (Refresh)</Button>
      </div>

      {/* Live Financial Summary Banner (Never 0.00 if orders exist) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        
        <div className="bg-card p-5 rounded-xl border shadow-sm space-y-1">
          <div className="flex justify-between items-center text-xs text-muted-foreground font-semibold">
            <span>ማተሚያ ቤቶች ያጠራቀሙት (Shop Earnings)</span>
            <Wallet className="w-4 h-4 text-emerald-500" />
          </div>
          <p className="text-2xl font-bold text-emerald-600">{totalShopEarned.toFixed(2)} ETB</p>
          <p className="text-[11px] text-muted-foreground">ከእያንዳንዱ ስራ ላይ የተሰላ</p>
        </div>

        <div className="bg-card p-5 rounded-xl border shadow-sm space-y-1">
          <div className="flex justify-between items-center text-xs text-muted-foreground font-semibold">
            <span>የአድሚን ገቢ እና አጠቃላይ ተቀናሽ</span>
            <DollarSign className="w-4 h-4 text-blue-500" />
          </div>
          <p className="text-2xl font-bold text-blue-600">{totalAdminCut.toFixed(2)} ETB</p>
          <p className="text-[11px] text-muted-foreground">የኮሚሽን + የሰርቨር/SMS ክፍያ</p>
        </div>

        <div className="bg-card p-5 rounded-xl border shadow-sm space-y-1">
          <div className="flex justify-between items-center text-xs text-muted-foreground font-semibold">
            <span>በጥሬ ገንዘብ የተሰበሰበ (Cash Debt)</span>
            <ArrowDownRight className="w-4 h-4 text-amber-500" />
          </div>
          <p className="text-2xl font-bold text-amber-600">{totalCashCollected.toFixed(2)} ETB</p>
          <p className="text-[11px] text-muted-foreground">ማተሚያ ቤቱ ለአድሚን የሚከፍለው እዳ</p>
        </div>
        ...
        <div className={`p-5 rounded-xl border shadow-sm space-y-1 ${netBalance >= 0 ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-amber-500/10 border-amber-500/30'}`}>
          ...
          <p className={`text-2xl font-bold ${netBalance >= 0 ? 'text-emerald-600' : 'text-amber-600'}`}>
            {Math.abs(netBalance).toFixed(2)} ETB
          </p>
          <p className="text-[11px] font-semibold text-foreground">
            {netBalance >= 0 ? "አድሚን ለህትመት ቤቱ የሚከፍለው" : "ህትመት ቤቱ ለአድሚን የሚከፍለው (እዳ)"}
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b pb-4">
        ...
        <Button 
          ...
          className="text-xs"
        >
          አብዴት ብቻ ያለቁ ({updateOnlyOrders.length})
        </Button>
        <Button 
          ...
          className="text-xs"
        >
          ህትመት ተጠናቆ ያለቀ ({printOrders.length})
        </Button>
        <Button 
          ...
          className="text-xs"
        >
          በጥሬ ገንዘብ (Cash Orders)
        </Button>
        <Button 
          variant={filter === "CHAPA" ? "default" : "outline"} 
          size="sm" 
      ...
      {filteredOrders.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredOrders.map((order) => {
            ...
            const isCash = order.paymentMethod === "CASH_TO_SHOP";
